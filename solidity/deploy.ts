...
async function main() {
  ...
  const cborDecoder = await ethers.getContractFactory("CBORDecoding");
  const marketContr = await ethers.getContractFactory("SnakeGameRewards", {
    libraries: {
        CBORDecoding: (await cborDecoder.deploy()).address,
    }
  });
  const marketDeploy = await marketContr.deploy("0x92981011F76bfC710feFB31D04BC8A6550648401");

  await marketDeploy.deployed();
  console.log("SnakeGameRewards at: ", marketDeploy.address);
